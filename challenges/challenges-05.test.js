...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    temp.push(recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ') + 1));
  }
  for (let i = 0; i < temp.length; i++) {
    result.push(temp[i].slice(temp[i].indexOf(' ') + 1));
  }
  return result;
}
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters < 0) {
    ...
    return str.slice(0, (str.length - numberOfCharacters + 1));
  }
};
